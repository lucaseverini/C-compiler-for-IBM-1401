...
public class Initializer 
{
	...
	public String generateLabel() 
	{
		...
		{
			label = ADDR_CONST(value.getValue(), isArrayMember);
		} 
		else if (value.getType().equals(Type.intType))
		{
			label = NUM_CONST(value.getValue(), isArrayMember);
		}
		else if (value.getType().equals(Type.charType))
		{
			label = CHAR_CONST(value.getValue(), isArrayMember);
		} 
		else 
		{
			// oops
	...
	public String generateCode() 
	{
		...
		if (subInitializers != null) 
		{
			if (variable.getType() instanceof ArrayType) 
			{
				ArrayType arrType = (ArrayType)variable.getType();
				
				Collections.reverse(subInitializers);
				Iterator<Initializer> iter = subInitializers.iterator();
				
				while(iter.hasNext())
				{
					String label = iter.next().generateLabel();
					
					if(arrType.lastElementLabel == null)
					{
						arrType.lastElementLabel = label;
					}
				}
/*				
