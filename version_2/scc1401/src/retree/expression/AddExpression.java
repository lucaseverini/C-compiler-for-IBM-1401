...
public class AddExpression extends Expression
{
	...
	public String generateCode(boolean valueNeeded)
	{
		...
		if (valueNeeded)
		{
			if (l.getType() instanceof PointerType)
			{
				Optimizer.addInstruction("Add", "", "MA", STACK_OFF(0), STACK_OFF(-r.getType().sizeof()));
				code += INS("Add", null, "MA", STACK_OFF(0), STACK_OFF(-r.getType().sizeof()));
			}
			else
			{
				Optimizer.addInstruction("Add", "", "A", STACK_OFF(0), STACK_OFF(-r.getType().sizeof()));
				code += INS("Add", null, "A", STACK_OFF(0), STACK_OFF(-r.getType().sizeof()));
			}
